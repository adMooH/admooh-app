...
async function addLicense(packageData) {
  const license = `/** @license adMooH APP v${packageData.version}
 *
...
`;
  await Promise.all(
    [
      './index.js',
      './esm/index.js',
      './umd/admooh-app.development.js',
      './umd/admooh-app.production.min.js',
    ].map(async file => {
      try {
  ...
  try {
    ...
    await Promise.all(
      [
        ...
        packageData.name === '@admooh-app/core' ? '../../README.md' : './README.md',
        '../../CHANGELOG.md',
        '../../LICENSE',
